...
export default class Smooth {
  constructor(opt = {}) {
    ...
    this.vs = this.vars.native
      ...
      : new vs({
          limitInertia:
            (this.options.vs && this.options.vs.limitInertia) || false,
          mouseMultiplier:
          ...
          preventTouch:
            (this.options.vs && this.options.vs.preventTouch) || true
        });
    this.dom = {
      listener: this.options.listener || document.body,
      section:
        this.options.section || document.querySelector('.vs-section') || null,
      scrollbar:
          ...
          : {
              ...
              el: create({
                ...
                styles: `vs-scrollbar vs-${
                  this.vars.direction
                } vs-scrollbar-${constructorName.toLowerCase()}`
              }),
              drag: {
  ...
  run() {
    ...
    if (!this.vars.native && !this.options.noscrollbar) {
      ...
      const bounds =
        this.vars.direction === 'vertical' ? this.vars.height : this.vars.width;
      const value =
        Math.abs(this.vars.current) / (this.vars.bounding / (bounds - size)) +
        size / 0.5 -
        size;
    ...
    if (this.callback && this.vars.current !== this.vars.last) {
      ...
      this.callbackEvents.forEach(callback => callback(this.vars.current));

      if (this.scrollingToTop === true && this.vars.current < 5) {
        this.topEvents.forEach(callback => callback(this.vars.current));
      }
    }
  ...
  getTransform(value) {
    return this.vars.direction === 'vertical'
  ...
  on(requestAnimationFrame = true) {
    ...
    this.vars.native
      ? event.on(node, 'scroll', this.debounce)
  ...
  off(cancelAnimationFrame = true) {
    const node =
      this.dom.listener === document.body ? window : this.dom.listener;
    this.vars.native
      ? event.off(node, 'scroll', this.debounce)
  ...
  onScrolledToTop(callback) {
    this.topEvents.push(callback);
  }
  ...
  resize() {
    ...
    if (!this.vars.native && !this.options.noscrollbar) {
      ...
      this.dom.scrollbar.drag.el.style[prop] = `${
        this.dom.scrollbar.drag.height
      }px`;
  ...
  destroy() {
    ...
    this.vars.direction === 'vertical'
      ? classes.remove(this.dom.listener, 'y-scroll')
      : classes.remove(this.dom.listener, 'x-scroll');
    this.vars.current = 0;
